              ...
              <div className="mb-3 flex items-center gap-2">
                {FILTERS.map((f) => (
                  <button key={f} type="button" onClick={() => setActiveFilter(f)}
                    className={`rounded-full px-3 py-1.5 text-xs font-semibold capitalize transition ${activeFilter === f ? 'bg-white text-neutral-900' : 'bg-white/5 border border-white/10 text-neutral-400 hover:bg-white/10'}`}>{f.replace('_', ' ')}</button>
                ))}
          ...
          <div className="flex items-center gap-2">
            {TEAM_FILTERS.map((f) => (
              <button key={f} type="button" onClick={() => setTeamFilter(f)}
                className={`rounded-full px-3 py-1.5 text-xs font-semibold capitalize transition ${teamFilter === f ? 'bg-white text-neutral-900' : 'bg-white/5 border border-white/10 text-neutral-400 hover:bg-white/10'}`}>{f}</button>
            ))}
            <span className="ml-auto text-xs text-neutral-500">{filteredTeams.length} team(s)</span>
          </div>
          {filteredTeams.length === 0 ? (
            <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-center backdrop-blur-sm"><p className="text-sm text-neutral-500">No teams found.</p></div>
          ) : (
      ...
      {activeTab === 'schedules' && (
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-white">Project Schedules</h3>
            <button type="button" onClick={() => setShowUploadForm(!showUploadForm)}
              className="rounded-lg bg-white px-4 py-2 text-sm font-semibold text-neutral-900 transition hover:bg-neutral-200">
          ...
          {schedules.length === 0 && !showUploadForm ? (
            <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-center backdrop-blur-sm"><p className="text-sm text-neutral-500">No schedules uploaded yet.</p></div>
          ) : (
            <div className="space-y-3">
              {schedules.map((s) => (
                <div key={s._id} className="rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-sm">
                  <div className="flex items-center justify-between cursor-pointer" onClick={() => setExpandedSchedule(expandedSchedule === s._id ? null : s._id)}>
                    ...
                    <span className="text-xs text-neutral-500">{expandedSchedule === s._id ? '▾' : '▸'}</span>
                  </div>
                  {expandedSchedule === s._id && (
                    <div className="mt-3 space-y-2 border-t border-white/10 pt-3">
                      {(s.tasks || []).map((t, idx) => (
                        <div key={t._id} className="rounded-lg bg-white/5 px-3 py-2 text-sm">
                          <div className="flex items-center justify-between">
                            <span className="text-neutral-200">Step {idx + 1}: {t.title}</span>
